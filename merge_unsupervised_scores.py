...
def merge_unsupervised_scores(path, metric, save_path):
	...
	og_results = [x.replace('/', '\\') for x in og_results]
	#print(f"og_results: {og_results}")
	# Read detectors and oracles scores
	metric_scores = metricsloader.read(metric.upper())
	metric_scores.index = [x.replace('/', '\\') for x in metric_scores.index]

	# Read all MS per split
	for filename, og_res in zip(split_names, og_results):
		og_res.index = [x.replace('/', '\\') for x in og_res.index]
		...
		for file in files:
			...
			current_classifier.index = [x.replace('/', '\\') for x in current_classifier.index]
